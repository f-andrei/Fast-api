...
def create_note_endpoint(note_data: Note, db: DBSession = Depends(get_db)):
    try:
        return create_note(session=db, note_data=note_data)
...
def get_note_endpoint(note_id: int, db: DBSession = Depends(get_db)):
    try:
        return get_note(session=db, note_id=note_id)
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail="Internal Server Error")
...
def update_note_endpoint(note_id: int, note_data: dict, db: DBSession = Depends(get_db)):
    try:
        updated_note = update_note(session=db, note_id=note_id, note_data=note_data)
        if updated_note:
...
def delete_note_endpoint(note_id: int, db: DBSession = Depends(get_db)):
    try:
        return delete_note(session=db, note_id=note_id)
    except Exception as e:
        print(e)
        db.rollback()
        raise HTTPException(status_code=500, detail="Internal Server Error")